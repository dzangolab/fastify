...
class W {
  ...
  async get(t) {
    const n = new p({
      Bucket: this.bucket,
      Key: t
    }), e = await this._storageClient.send(n), s = e.Body, o = await B(s);
    return {
...
class ht extends I {
  ...
  upload = async (t) => {
    const { fileContent: n, fileFields: e } = t.file, { filename: s, mimetype: o, data: a } = n, {
      path: r = "",
      bucket: u = "",
      bucketChoice: c,
      filenameResolutionStrategy: l
    } = t.options || {}, b = j(s);
    this.fileExtension = b, this.path = r, this.s3Client.bucket = v(u, e?.bucket, c) || "";
    let h = this.key;
    const w = await this.s3Client.isFileExists(h), T = l || this.config.s3.filenameResolutionStrategy;
    if (w)
      switch (T) {
        case $:
        ...
        case M: {
          ...
          this.filename = _, h = this.key;
          break;
        }
      }
    if (!await this.s3Client.upload(a, h, o))
      return;
    const C = {
      ...e && { ...e },
      ...e?.uploadedAt && {
...
`;
  ...
  console.log("s3 graphql"), i.log.info("Registering fastify-s3 plugin");
  ...
  }), console.log("before s3 graphql"), e.graphql?.enabled && await i.register(q, {
    maxFileSize: e.s3.fileSizeLimitInBytes || Number.POSITIVE_INFINITY
  }), console.log("after s3 graphql"), n();
}, dt = d(G), J = (i, t, n) => {
  i.hasContentTypeParser("multipart") || i.addContentTypeParser("multipart", (e, s, o) => {
    e.config.graphql?.enabled && e.routerPath.startsWith(e.config.graphql.path) ? (e.mercuriusUploadMultipart = !0, o(null)) : H(e, s, o);
  }), n();
}, ft = d(J);
export {
  ht as FileService,
