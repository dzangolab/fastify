...
import { getSignedUrl as F } from "@aws-sdk/s3-request-presigner";
import O from "@fastify/multipart";
import d from "fastify-plugin";
import { sql as g } from "slonik";
import { processRequest as U } from "graphql-upload-minimal";
...
class W {
  ...
  async generatePresignedUrl(t, n, e = 3600) {
    ...
    return await F(this._storageClient, s, {
      expiresIn: e
    });
  }
  /**
...
class ut extends I {
  ...
  upload = async (t) => {
    ...
    let h = this.key;
    const b = await this.s3Client.isFileExists(h), T = l || this.config.s3.filenameResolutionStrategy;
    if (b)
      switch (T) {
        case $:
    ...
    if (!await this.s3Client.upload(o, h, a))
      return;
    const C = {
      ...e && { ...e },
      ...e?.uploadedAt && {
...
`;
  ...
  i.addHook("preValidation", async (e, s) => {
    e.graphqlFileUploadMultipart && (e.body = await U(e.raw, s.raw, t));
  }), n();
}, G = d(X, {
  fastify: ">= 4.x",
  ...
  i.hasContentTypeParser("multipart") || i.addContentTypeParser("multipart", (e, s, a) => {
    e.config.graphql?.enabled && e.routerPath.startsWith(e.config.graphql.path) ? (e.graphqlFileUploadMultipart = !0, a(null)) : H(e, s, a);
  }), n();
}, dt = d(J);
export {
  ut as FileService,
