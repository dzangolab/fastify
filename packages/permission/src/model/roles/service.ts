  ...
  implements Service<Role, RoleCreateInput, RoleUpdateInput>
{
  ...
  create = async (data: RoleCreateInput) => {
    ...
    return this.findById(result.id as number) as Promise<Role | undefined>;
  };
  ...
  delete = async (id: number | string): Promise<Role | null> => {
    ...
    // TODO check if role is assigned, if yes throw an error.
  ...
  setRolePermissions = async (
    ...
    const query = this.factory.getSetRolePermissionsSql(roleId, permissionIds);
